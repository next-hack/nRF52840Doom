...
void P_PlayerThink(player_t *player)
{
    ...
    if (getMobjSubsector(player->mo)->sector->special)
    ...
    if (cmd->buttons & BT_CHANGE)
    {
        ...
        if (player->weaponowned[newweapon] && newweapon != player->readyweapon)
            ...
            if ((newweapon != wp_plasma && newweapon != wp_bfg) || (_g->gamemode != shareware))
    ...
    player->fixedcolormap =
            player->powers[pw_invulnerability] > 4 * 32 || (player->powers[pw_invulnerability] & 8) ? INVERSECOLORMAP : player->powers[pw_infrared] > 4 * 32 || (player->powers[pw_infrared] & 8);
}
