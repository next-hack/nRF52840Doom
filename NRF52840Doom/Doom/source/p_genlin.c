    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        switch (Targ)
        {
            ...
            case FtoNnF:
                floor->floordestheight =
                        Dirn ? P_FindNextHighestFloor(sec, sec->floorheight) : P_FindNextLowestFloor(sec, sec->floorheight);
                break;
            case FtoLnC:
            ...
            case FtoC:
                ...
                break;
            case FbyST:
                floor->floordestheight = (getFloorMoveSector(floor)->floorheight >> FRACBITS) + floor->direction * (P_FindShortestTextureAround(secnum) >> FRACBITS);
                ...
                break;
            case Fby24:
                floor->floordestheight = getFloorMoveSector(floor)->floorheight + floor->direction * 24 * FRACUNIT;
                break;
            case Fby32:
                floor->floordestheight = getFloorMoveSector(floor)->floorheight + floor->direction * 32 * FRACUNIT;
                break;
            default:
                break;
        }
        ...
        if (ChgT)   // if a texture change is indicated
        {
            if (ChgM) // if a numeric model change
            {
                ...
                //is a ceiling type
                sec = (Targ == FtoLnC || Targ == FtoC) ? P_FindModelCeilingSector(floor->floordestheight, secnum) : P_FindModelFloorSector(floor->floordestheight, secnum);
                if (sec)
                {
                    floor->texture = sec->floorpic;
                    switch (ChgT)
...
int EV_DoGenCeiling(const line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        sec->ceilingdata_sptr = getShortPtr(ceiling); //jff 2/22/98
        ...
        switch (Targ)
        {
            ...
            case CtoNnC:
                targheight =
                        Dirn ? P_FindNextHighestCeiling(sec, sec->ceilingheight) : P_FindNextLowestCeiling(sec, sec->ceilingheight);
                break;
            case CtoHnF:
...
int EV_DoGenLift(const line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        plat->sector = sec;
        plat->sector->floordata_sptr = getShortPtr(plat);
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        do
        {
            ...
            for (i = 0; i < sec->linecount; i++)
            {
                ...
                if (!LN_BACKSECTOR((getSectorLineByIndex(sec, i))))
                    continue;

                tsec = LN_FRONTSECTOR((getSectorLineByIndex(sec, i)));
        ...
        switch (Kind)
        {
            case OdCDoor:
                ...
                if (door->topheight != sec->ceilingheight)
                    S_StartSound2(&door->sector->soundorg,
                            Sped >= SpeedFast ? sfx_bdopn : sfx_doropn);
            ...
            case CdODoor:
                ...
                door->direction = -1;
                S_StartSound2(&door->sector->soundorg,
                        Sped >= SpeedFast ? sfx_bdcls : sfx_dorcls);
