    ...
    for (i = -1; (i = P_FindSectorFromLineTag(line, i)) >= 0;)
    {
        ...
        while ((th = P_NextThinker(th)) != NULL)
            if (th->function == P_MobjThinker)
            {
                ...
                if (m->type == MT_TELEPORTMAN && getMobjSubsector(m)->sector - _g->sectors == i)
                    return m;
            }
    }
    return NULL;
...
int EV_Teleport(const line_t *line, int side, mobj_t *thing)
{
    ...
    if ((m = P_TeleportDestination(line)) != NULL)
    {
        ...
        S_StartSound(P_SpawnMobj(m->x + 20 * finecosine[m->angle >> ANGLETOFINESHIFT], m->y + 20 * finesine[m->angle >> ANGLETOFINESHIFT], thing->z, MT_TFOG), sfx_telept);
...
int EV_SilentLineTeleport(const line_t *line, int side, mobj_t *thing, boolean reverse)
{
    ...
    for (i = -1; (i = P_FindLineFromLineTag(line, i)) >= 0;)
        if ((l = _g->lines + i) != line && LN_BACKSECTOR(l))
        {
            ...
            thing->z = z + _g->sectors[_g->sides[l->sidenum[stepdown]].sector_num].floorheight;
