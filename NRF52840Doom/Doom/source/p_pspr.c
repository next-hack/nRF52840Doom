    ...
    do
        switch (*prefer++)
        {
            ...
            case 5:
                ...
                break;
            case 6:
                if (player->weaponowned[wp_plasma] && player->ammo[am_cell] && _g->gamemode != shareware)
                    newweapon = wp_plasma;
                break;
            case 7:
                if (player->weaponowned[wp_bfg] && _g->gamemode != shareware && player->ammo[am_cell] >= (40))
                    newweapon = wp_bfg;
                break;
            case 8:
                ...
                break;
            case 9:
                if (player->weaponowned[wp_supershotgun] && _g->gamemode == commercial && player->ammo[am_shell] >= (2))
...
int P_CheckCanSwitchWeapon(weapontype_t weapon, player_t *player)
{
    switch (weapon)
    {
        ...
        case wp_supershotgun:
        {
            ...
            break;
        case wp_nochange:		// don't do anything
        default:
            break;
    }
...
void A_WeaponReady(player_t *player, pspdef_t *psp)
{
    ...
    if (getMobjState(player->mo) == &states[S_PLAY_ATK1] || getMobjState(player->mo) == &states[S_PLAY_ATK2])
...
void A_BFGSpray(mobj_t *mo)
{
    ...
    for (i = 0; i < 40; i++)  // offset angles from its attack angle
    {
        ...
        // killough 8/2/98: make autoaiming prefer enemies
        if ((P_AimLineAttack(getTarget(mo), an, 16 * 64 * FRACUNIT, MF_FRIEND), !_g->linetarget))
            P_AimLineAttack(getTarget(mo), an, 16 * 64 * FRACUNIT, 0);
